...
date: "2026-04-12"
description: "TermDeck captures. Mnestra stores. Rumen reasons. The brain doesn't sleep when you close your laptop."
tags: ["termdeck", "mnestra", "rumen", "developer-tools"]
draft: false
---
...
TermDeck pipes structured events — session starts, commands executed, status changes, file edits detected — into Mnestra. Mnestra indexes them with full-text search and vector embeddings. Rumen reads from Mnestra on a schedule, synthesizes cross-session patterns, and writes distilled insights back into Mnestra's memory store.

The next morning, when a new coding session starts, Mnestra surfaces both raw memories and Rumen's synthesized insights. The context window is seeded with things you forgot you knew.
...
| | **Karpathy (Obsidian wiki)** | **Mem0** | **Ruflo** | **TermDeck + Mnestra + Rumen** |
|---|---|---|---|---|
...
| **MCP support** | No | No | No | Mnestra is a native MCP server — works with any compatible client |
...
<Callout variant="insight">The key differentiator is the closed loop: TermDeck captures passively, Mnestra stores durably, and Rumen reasons autonomously — even after you close your laptop. No other approach connects all three layers.</Callout>

## Why three repos, not one

**Unix philosophy. Each tool does one thing well.**

A developer who wants a better terminal multiplexer can install TermDeck without caring about memory systems. Someone building an MCP memory server can use Mnestra without needing terminals. A team interested in async pattern synthesis can study Rumen's approach independently.
...
- **Mnestra**: [github.com/jhizzard/mnestra](https://github.com/jhizzard/mnestra) — persistent memory MCP server
