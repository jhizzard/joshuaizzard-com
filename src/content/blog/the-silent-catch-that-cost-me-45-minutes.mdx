...
My first instinct was environment variables. Vercel's dashboard showed all the expected keys — `DATABASE_URL`, `NEXTAUTH_SECRET`, `NEXTAUTH_URL`. I copied them into a local `.env`, ran the route locally, and it worked fine. So not env vars.
...
The Prisma error was the proximate cause. The real cause was this pattern, which I had written months earlier and never thought about again:
...
The immediate fix was mechanical. Every catch block in every API route got the same treatment:
...
I adopted a **`[tag]` prefix convention** across all my projects. Every module gets a short, greppable label:
...
Fixing my own code was not enough. I needed to make sure this pattern could never reappear in any project I work on. I added an ESLint rule that flags bare catch blocks with no logging:
