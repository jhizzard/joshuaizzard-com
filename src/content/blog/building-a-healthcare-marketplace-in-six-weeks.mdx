...
**When a patient submits a service request, relevant providers see it and can bid in real time.**

The bidding engine handles bid validation, ranking, and expiration. Once a patient accepts a bid, Stripe Connect processes the payment using a capture-then-refund pattern — funds are authorized and captured, but the platform holds them through an inspection window so the patient can flag issues or cancel if needed. Providers receive payouts through connected Stripe accounts.
