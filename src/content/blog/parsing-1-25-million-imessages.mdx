...
<Callout variant="tip">Always open Apple's chat.db with SQLite's read-only VFS mode (`file:chat.db?mode=ro`). This is a kernel-level guarantee that no write can occur, regardless of what your application code does. Do not rely on application-level read-only flags alone.</Callout>
...
<PullQuote>Apple built a 4GB database with a custom binary format and then shipped a client that can barely read it.</PullQuote>
